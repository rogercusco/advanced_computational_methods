# tree = buildtree(5, iris, 5, 5, "ME"); tree

# result = cTree(Species ~ Sepal.Length + Sepal.Width +Petal.Length +Petal.Width, iris, 5, 5, "ME")
...
cTree <- function( formula, data, depth, minPoints, costFnc = "Entropy") {
  ...
  y = ncol(X)
    ...
    for(i in as.numeric(terminal_names)) {
      ...
      # print(sort(table(subset_node[,y]), decreasing=TRUE))
    }
